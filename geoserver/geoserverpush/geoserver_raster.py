...
class GeoserverRaster:
    ...
    def __init__(self):
        self.source_tif = self.base_name = self.display_name = self.native_layer_name = self.uuid = self.srs = self.metadata = ""
    ...
    def load_bath_from_database_entry(self, source_tif_entry):
        ...
        self.native_layer_name = re.sub(
            ".tif", "", re.sub(".*/", "", self.source_tif))
        self.uuid = "{0}-OV".format(source_tif_entry.UUID)
        self.srs = source_tif_entry.srs
        self.metadata = source_tif_entry.metadata
